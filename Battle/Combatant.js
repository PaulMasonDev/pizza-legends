class Combatant {
  ...
  update(changes = {}) {
    ...
    //Update active flag to show the correct hud and pizza
    this.hudElement.setAttribute("data-active", this.isActive);
    this.pizzaElement.setAttribute("data-active", this.isActive);

    //Update HP and XP fills
  ...
  getPostEvents() {
    ...
    if (this.status?.type === "clumsy") {
      return [
        { type: "textMessage", text: "Sliping and slidin' with Olive Oil!" },
      ];
    }
    return [];
  }
  ...
  decrementStatus() {
    if (this.status?.expiresIn > 0) {
      ...
      if (this.status.expiresIn === 0) {
        const text = `Status expired! No longer ${this.status.type}.`;
        this.update({
          status: null,
        });
        return {
